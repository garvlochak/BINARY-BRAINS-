...
GlaucoSense/
├── firmware/
│   ├── tests/                   Standalone bring-up sketches, run in order 01 -> 08
...
2. Run the test sketches in `firmware/tests/` in order (01 through 08) to
   confirm each piece of hardware works in isolation - see **Next steps**
   below for the full order.
3. Open `firmware/GlaucoSense_Final/GlaucoSense_Final.ino` in the Arduino IDE
   (it will pick up the other .h/.cpp files in the same folder automatically).
4. Upload it, then open the Serial Monitor at 115200 baud.
5. At boot, keep the pressure line open to atmosphere (pump/valve off) while
...
1. Run firmware tests **01 -> 08** in `firmware/tests/`, one at a time, to
   confirm each piece of hardware individually:
   `01_esp32_test -> 02_pump_test -> 03_valve_test -> 04_pressure_raw_test ->
   05_pressure_calibration -> 06_tof_test -> 07_dual_sensor_test -> 08_oled_test`
2. Do a reservoir/pump calibration pass - confirm `chargeReservoir()` reaches
   `TARGET_PRESSURE_KPA` within `PUMP_TIMEOUT_MS` on your actual pump/reservoir
   setup, and adjust those values in `config.h` if needed.
3. Run a full trial on `GlaucoSense_Final` end-to-end on the artificial
